...
class NameSpaceModel:
    ...
    def fill_base(self):
        ...
        if not os.path.exists(self.config[_experiment][_performance]):
            os.makedirs(self.config[_experiment][_performance])
        # manage_directories(self.config[_experiment][_recs], self.config[_experiment][_weights],
        #                    self.config[_experiment][_performance])

        for p in [_data_config, _dataset, _performance, _logger_config,
                  _log_folder, _dataloader, _evaluation,
                  _print_triplets, _config_test, _binarize,
                  _version]:
            ...
            '''
                ...
                setattr(self.base_namespace, p, self.config[_experiment][p])
            '''
            if p == _evaluation and self.config[_experiment].get(p, {}):
